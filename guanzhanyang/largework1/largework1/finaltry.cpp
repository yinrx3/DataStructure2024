...
int what_to_assigment(int n,queue<Clause> clause_left,vector<literal> solution)
{
    ...
    while(!clause_left.empty()){
        ...
        for(int i=0;i<3;++i){
            if(solution[abs(front[i])-1].val==-1){
                if(front[i]>0){
                    ++solution[abs(front[i])-1].plus;
                }
                else{
                    ++solution[abs(front[i])-1].minus;
                }
            }
        }
        clause_left.pop();
    ...
    //目的是找到单一极性出现次数最多的
    int maxscore=0;
    int maxindex=0;//存储的变量名字不是位序
    for(int i=0;i<n;++i){
        solution[i].score=max(solution[i].plus,solution[i].minus);
        if(solution[i].val==-1){
            if(solution[i].score>maxscore){
                maxscore=solution[i].score;
                maxindex=i+1;
            }
        }
    }
    if(solution[maxindex-1].plus>=solution[maxindex-1].minus){
        return maxindex;
    }
    else{
        return -maxindex;
