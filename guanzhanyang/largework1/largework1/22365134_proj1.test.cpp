...
int index_name_order(int pname,vector<literal> literal_solution){
    int j=0;
    while(literal_solution[j].name!=pname){
        ++j;
    }
    return j;
}
...
int if_Clause_Satisfied(Clause clause,vector<literal> literal_solution)//测试在目前赋值下单个字句能否被满足，包括当前未赋值情况
{
    ...
    for(int i=0;i<3;++i){
        clause_result_val.push_back(literal_solution[index_name_order(abs(clause[i]),literal_solution)].val);
    }
...
void first_assigment_to_xi(queue<Clause> Clause_not_Satisfied,int xi,vector<literal> &literal_solution)//对xi应该赋值为正极性还是负极性
{
    ...
    while(!Clause_not_Satisfied.empty()){
        ...
        for(int j=0;j<3;++j){
            if(clause_now_considering[j]==literal_solution[xi-1].name){++numplus;}
            if(clause_now_considering[j]==-literal_solution[xi-1].name){++numminus;}
        }
