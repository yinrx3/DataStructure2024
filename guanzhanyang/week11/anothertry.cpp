...
class AMGraph{ //we need the number of vertices fixed if we are using this approach
    ...
    public:
        ...
        void caculatedegree(int* inDegrees){
            ...
            for(int i=0;i<numVer;++i){
                if(directed){
                    for(int j=0;j<numVer;++j){if(adjMatrix[i][j]==1) ++inDegrees[j];}
                }
                else{
                    for(int j=i+1;j<numVer;++j){
		...
		// question 4
        bool loopTest() {
            //采用不断删除结点的方法，如果是无向图就删除度数是1的结点，如果是有向图，就删除入度是0的结点
            int* inDegrees=new int[numVer];
            this->caculatedegree(inDegrees);
            LinkQueue<int> q;
            //为了方便处理这里复制一遍邻接矩阵·1
            ...
            int noderemove=0;
            int maxgo=numVer*numVer*numVer;
            int k=0;
            while(!q.isEmpty()&&k<=maxgo){
                int nodeIndex=q.deQueue();
                ++k;
                if(directed){
                    //处理有向图
                    if(inDegrees[nodeIndex]==0)
                    {
                        ++noderemove;
                        for(int i=0;i<numVer;++i){
                            if(copyMatrix[nodeIndex][i]==1){
                                --inDegrees[i];
                                copyMatrix[nodeIndex][i]=0;
                                copyMatrix[i][nodeIndex]=0;
                            }
                        }
                    }
                    else{
                    ...
                    else if(inDegrees[nodeIndex]==1){
                        ++noderemove;
                        for(int i=0;i<numVer;++i){
                            if(copyMatrix[nodeIndex][i]==1&&copyMatrix[i][nodeIndex]==1){
                                --inDegrees[i];
                                --inDegrees[nodeIndex];
                                copyMatrix[nodeIndex][i]=0;
                                copyMatrix[i][nodeIndex]=0;
