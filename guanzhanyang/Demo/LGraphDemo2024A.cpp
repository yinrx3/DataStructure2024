...
class LGraph{ //we need the number of vertices fixed if we are using this approach
    ...
    private:
        ...
        void BFShelper(int st, bool* visited){
            ...
            while(!q.isEmpty()){
                ...
                for(int i=0;i<tmpNode->outs->getLength();i++){
                    graphNode<T>* eNode = tmpNode->outs->getEleAtPos(i);
                    int eInd = verList.locateEle(eNode);
                    if(visited[eInd]) continue;
                    visited[eInd] = true;
                    cout << eNode->data << " ";
                    q.enQueue(eInd);
                }
		...
		void DFShelper(int st, bool* visited){
            visited[st] = true;
            graphNode<T>* sNode = verList.getEleAtPos(st);
            cout << sNode->data << " ";
            for(int i=0;i<sNode->outs->getLength();i++){
                graphNode<T>* eNode = sNode->outs->getEleAtPos(i);
                int eInd = verList.locateEle(eNode);
                if(visited[eInd]) continue;
                DFShelper(eInd,visited);
            }
		}
    ...
    public:
        ...
        ~LGraph(){
            for(int i=0;i<numVer;i++){
                graphNode<T>* node = verList.getEleAtPos(i);
                delete node->ins;
                delete node->outs;
                delete node;
            }
        ...
        void addEdge(T st, T en){
            int sInd = findIndex(st);
            int eInd = findIndex(en);
            graphNode<T>* sNode = verList.getEleAtPos(sInd);
            graphNode<T>* eNode = verList.getEleAtPos(eInd);
            if(sNode->outs->locateEle(eNode)!=-1) return;
        ...
        void addVertex(T x){
            ...
            node->data = x;
            node->ins = new VecList<graphNode<T>*>;
            node->outs = new VecList<graphNode<T>*>;
            verList.insertLast(node);
        }
        ...
        int** getMatrix(){
            int** adjMatrix = new int*[numVer];
            for(int i=0;i<numVer;i++){
                adjMatrix[i] = new int[numVer];
                graphNode<T>* startNode = verList.getEleAtPos(i);
                for(int j=0;j<numVer;j++){
                    adjMatrix[i][j] = 0;
                    graphNode<T>* endNode = verList.getEleAtPos(j);
                    if(startNode->outs->locateEle(endNode)!=-1)
                        adjMatrix[i][j] = 1;
                }
            }
            return adjMatrix;
