...
class LGraph{ //we need the number of vertices fixed if we are using this approach
    ...
    public:
        ...
        void printStrongCC(){
            int* compNum=new int[numVer];//记录每一个结点所在强连通分量的编号
            //初始时刻，每个节点自己就在一个强连通分量里面
            for(int i=0;i<numVer;++i){
                compNum[i]=i;
            }
            //下面依次判断，两个节点是否在同一个强连通分量重，如果在同一个强连通分量里面，就把两个点编号改成较小的点的编号
            
            for(int i=0;i<numVer;++i){
                for(int j=i+1;j<numVer;++j){
                    if(ifInSameCC(i,j)){
                        int combinedNum=(compNum[i]<compNum[j]?compNum[i]:compNum[j]);
                        compNum[i]=combinedNum;
                        compNum[j]=combinedNum;
                    }
                }
            }
            ...
            int posprint=0;
            for(int i=0;i<numVer;++i){
                //如果有编号为i的就打印,打印完++posprint
                bool isfind=false;
                for(int j=0;j<numVer;++j){
                    if(compNum[j]==i){
                        cout<<"Component "<<posprint<<": ";
                        ++posprint;
                        isfind=true;
                        break;
                    }
                ...
                for(int j=0;j<numVer;++j){
                    if(compNum[j]==i){
                        cout<<verList.getEleAtPos(j)->data<<" "; 
                    }
                }
                cout<<endl;
            }       
        ...
        bool ifInSameCC(int uIndex,int vIndex){
            if(ifRoad(uIndex,vIndex)&&ifRoad(vIndex,uIndex)) return true;
            return false;
        }

        bool ifRoad(int uIndex,int vIndex){
            ...
            q.enQueue(uIndex);
            while(!q.isEmpty()){
                int tmpIndex=q.deQueue();
                for(int i=0;i<numVer;++i){
                    if(adjMatrix[tmpIndex][i]==1&&!visited[i]){
                        q.enQueue(i);
                        visited[i]=true;
                        if(i==vIndex){
                            return true;
