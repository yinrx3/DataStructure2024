...
int ifmatch(string* codeSet,string p,int numCodes);
int ifmatch(string* codeSet,string p,int numCodes){
    for(int i=0;i<numCodes;++i){
        if(p==codeSet[i]){
            return i;
...
void printDecode(char* charSet, string* codeSet, int numCodes, string encodedStr){
    string result;
    while(!encodedStr.empty()){
        int num=1;
        while(ifmatch(codeSet,encodedStr.substr(0,num),numCodes)==-1){
            ++num;
        }
        int cur=ifmatch(codeSet,encodedStr.substr(0,num),numCodes);
        result+=charSet[cur];
        encodedStr=encodedStr.substr(num,encodedStr.size()-num);
    }
    cout<<result;
}
...
void test2(){ // test for validity of isPrefixCode
