...
int weightedPathLength(BinTree<int>* bt){
    if(bt==nullptr){
        return 0;
    }
    int addPathLength=0;
    LinkQueue<BinTree<int>*> que;
    que.enQueue(bt);
    int level=-1;
    while(!que.isEmpty()){
        int size=que.getLength();
        level++;
        for(int i=0;i<size;++i){
            BinTree<int>* node=que.deQueue();
            if(node->getLChild()==nullptr&&node->getRChild()==nullptr){
                addPathLength+=level*node->getData();
            }
            if(node->getLChild()!=nullptr){que.enQueue(node->getLChild());}
            if(node->getRChild()!=nullptr){que.enQueue(node->getRChild());}
        }
    }
    return addPathLength;
}
